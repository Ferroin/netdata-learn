...
- On highlight, get easy access to [Metric Correlations](/docs/agent/metric-correlations) to see other metrics with similar patterns
...
[write new alerts](/docs/agent/src/health/reference), or play around
with the [API](/docs/agent/src/web/api).
...
Netdata also relies on contexts for [alert configuration](/docs/agent/src/health/reference) (the [`on` line](/docs/agent/src/health/reference#alert-line-on)).
...
For more details on each, you can refer to our Agent's HTTP API details on [Data Queries - Data Grouping](/docs/agent/src/web/api/queries#data-grouping).
...
- Running [metric correlations](/docs/agent/metric-correlations) to filter metrics that also show something different in the selected period, vs the previous one
