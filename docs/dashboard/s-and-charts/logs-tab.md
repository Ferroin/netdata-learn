...
[Logs Management](/docs/agent/category-overview-pages/working-with-logs).
...
|:-------|:---------|:--------------|:----------|
| `systemd-journal` | Linux | systemd journal files: system, user, namespace, and remote journals, merged by default | [Systemd Journal Plugin Reference](/docs/agent/src/collectors/systemd-journal.plugin) |
| `windows-events` | Windows | Windows event channels, auto-detected and aggregated, including forwarded-events channels on a Windows Event Collector | [Windows Events Plugin Reference](/docs/agent/src/collectors/windows-events.plugin) |
| `macos-logs` | macOS | The native unified log store, through Apple's OSLog framework | [macOS Logs Plugin Reference](/docs/agent/src/collectors/macos-logs.plugin) |
| `otel-logs` | Any, via OTLP | Logs received by the OpenTelemetry plugin and stored in Netdata's indexed log store | [OpenTelemetry plugin reference](/docs/agent/src/crates/otel-plugin) |
| `snmp:traps` | Any | Journal-compatible files Netdata writes for the traps received by its SNMP traps collector | [SNMP Trap Logs](/docs/agent/logs/snmp-trap-logs) |

Application text files appear under `systemd-journal` when converted with log2journal, or under `otel-logs` when
shipped through an OpenTelemetry Collector; see [Text Files to Journals](/docs/agent/logs/text-files-to-journals).
Network flows are stored the same way as SNMP traps but have their own view,
[Network Flows](/docs/agent/logs/network-flows), instead of the Logs tab.

:::note

On Linux systems without systemd, such as Alpine Linux, the `systemd-journal` source is unavailable. Convert and push
logs to a remote `systemd-journal-remote` with [`systemd-cat-native --url`](/docs/agent/src/libnetdata/log/systemd-cat-native),
or send them through [OpenTelemetry](/docs/agent/opentelemetry/otlp-ingestion).
...
- Where journal sequence numbers are available, estimates use them for precision. See
  [Performance at scale](/docs/agent/src/collectors/systemd-journal.plugin#performance-at-scale) for the math.
...
- Platform requirements, packages, and container settings are documented in each plugin's reference:
  [systemd journal](/docs/agent/src/collectors/systemd-journal.plugin#prerequisites),
  [Windows events](/docs/agent/src/collectors/windows-events.plugin#prerequisites),
  [macOS logs](/docs/agent/src/collectors/macos-logs.plugin#prerequisites),
  [OpenTelemetry](/docs/agent/src/crates/otel-plugin).
...
your infrastructure. See [Netdata AI](/docs/agent/category-overview-pages/machine-learning-and-assisted-troubleshooting).

## Where to next

- [Logs Management](/docs/agent/category-overview-pages/working-with-logs) — where logs are stored, how to decide per
  source, and the current limitations.
- [Text Files to Journals](/docs/agent/logs/text-files-to-journals) — bring application log files into these sources.
