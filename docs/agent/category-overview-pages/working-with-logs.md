...
[Log Storage and Retention](/docs/agent/logs/log-storage-and-retention).
...
| Logs that must outlive the node: audit trails, forensics, retention mandates | [Centralize these sources with OpenTelemetry](/docs/agent/logs/centralizing-logs-with-opentelemetry) into Netdata's log store; leave everything else in place. |
...
| Kubernetes and containers | Run the [OpenTelemetry Collector in the cluster](/docs/agent/opentelemetry/logs-collection#kubernetes-and-containers) and ship container logs to Netdata's log store. |
...
| Network devices sending syslog | An [OpenTelemetry Collector syslog receiver](/docs/agent/npm/syslog) forwarding to Netdata. |
| Network devices sending SNMP traps, NetFlow, sFlow, or IPFIX | Netdata receives them directly and writes journal-compatible files on the receiving node; see [SNMP Trap Logs](/docs/agent/logs/snmp-trap-logs) and [Network Flows](/docs/agent/logs/network-flows). |
| macOS | In place. macOS has no OS-native log forwarding; to centralize, use the [OpenTelemetry Collector's macOS receiver](/docs/agent/opentelemetry/logs-collection#macos-unified-log). |
...
|:-------|:---------|:--------|:-------------------|
| [systemd journal](/docs/agent/src/collectors/systemd-journal.plugin) | Linux | Native journal files: system, user, namespace, and remote journals | Logs tab, `systemd-journal` |
| [Windows Event Log](/docs/agent/src/collectors/windows-events.plugin) | Windows | Native event channels, including the forwarded-events channels on a collector | Logs tab, `windows-events` |
| [macOS unified log](/docs/agent/src/collectors/macos-logs.plugin) | macOS | The native unified log store | Logs tab, `macos-logs` |
| [OpenTelemetry logs](/docs/agent/src/crates/otel-plugin) | Any, via OTLP | Netdata's indexed log store | Logs tab, `otel-logs` |
| Text log files | Any | A journal, through [log2journal](/docs/agent/src/collectors/log2journal); or Netdata's log store, through an [OpenTelemetry Collector](/docs/agent/opentelemetry/logs-collection) | Logs tab, `systemd-journal` or `otel-logs` |
| [Network device syslog](/docs/agent/npm/syslog) | Network devices | Netdata's log store, through an OpenTelemetry Collector | Logs tab, `otel-logs` |
| [SNMP traps](/docs/agent/logs/snmp-trap-logs) | Network devices | Journal-compatible files written by Netdata under its log directory | Logs tab, `snmp:traps` (SNMP Trap Logs) |
| [Network flows](/docs/agent/logs/network-flows) (NetFlow, sFlow, IPFIX) | Network devices | Journal-compatible files written by Netdata under its cache directory, in four time tiers | The Network Flows view |
...
[Managing Logs](/docs/dashboards-and-charts/logs-tab).
...
- **Alerts are evaluated on metrics, not on log content.** To alert on a log pattern, derive a metric from it, for
  example with [logs-to-metrics](/docs/agent/opentelemetry/logs-to-metrics), and alert on that metric.
...
- [Managing Logs](/docs/dashboards-and-charts/logs-tab) — how logs are organized, queried, and explored: sources,
  filters, full-text search, histograms, live tail, and query behavior at scale.
- [Systemd Journal Logs](/docs/agent/src/collectors/systemd-journal.plugin) — the journal reference,
  [Forward Secure Sealing](/docs/agent/src/collectors/systemd-journal.plugin/forward_secure_sealing), and
  [Logs Centralization Points](/docs/agent/observability-centralization-points/logs-centralization-points-with-systemd-journald)
  for environments that already aggregate journals.
- [Windows Event Logs](/docs/agent/src/collectors/windows-events.plugin) — event channels on nodes and on Windows
  Event Collectors.
- [macOS Unified Logs](/docs/agent/src/collectors/macos-logs.plugin) — the unified log store on macOS nodes.
- [Text Files to Journals](/docs/agent/logs/text-files-to-journals) — application log files, with
  [log2journal](/docs/agent/src/collectors/log2journal) and
  [systemd-cat-native](/docs/agent/src/libnetdata/log/systemd-cat-native).
- [Centralizing Logs with OpenTelemetry](/docs/agent/logs/centralizing-logs-with-opentelemetry) — which sources to
  centralize and what to set up; the Collector recipes live in the OpenTelemetry pages.
- [Log Storage and Retention](/docs/agent/logs/log-storage-and-retention) — retention settings per tier, offloading to
  object storage, and sizing.
- [SNMP Trap Logs](/docs/agent/logs/snmp-trap-logs) and [Network Flows](/docs/agent/logs/network-flows) — the journals
  Netdata writes for network devices.
- Integrations — the per-source integration cards.
- Related: the OpenTelemetry section — [OTLP ingestion](/docs/agent/opentelemetry/otlp-ingestion),
  [logs collection](/docs/agent/opentelemetry/logs-collection),
  [transformations](/docs/agent/opentelemetry/transformations),
  [logs-to-metrics](/docs/agent/opentelemetry/logs-to-metrics), and the
  [OpenTelemetry plugin reference](/docs/agent/src/crates/otel-plugin).
