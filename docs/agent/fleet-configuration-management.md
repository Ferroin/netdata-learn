...
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------- | -------------- | -------------------------------------------------------------------------------------- |
| **Auto-deploy**                    | ✅ [kickstart.sh](/docs/agent/packaging/installer/methods/kickstart)¹ | ✅ [Helm](/docs/netdata-agent/installation/kubernetes) | ✅ kickstart.sh | ✅ kickstart.sh | ✅ [MSI](/docs/netdata-agent/installation/windows)² (silent) |
...
1. **kickstart.sh**: [Universal installer script](/docs/agent/packaging/installer/methods/kickstart) that auto-detects the best installation method
...
| **Hybrid (Best)** | Most organizations                    | Base config in Git, credentials/thresholds via [UI](/docs/netdata-agent/configuration/dynamic-configuration-manager) |
...
Netdata ensures upgrades don't break existing configurations and data - Netdata maintains compatibility across versions allowing seamless updates (see [Netdata Infrastructure](/docs/welcome-to-netdata) for architectural details)
...
For organizations with established DevOps practices, Netdata fully supports configuration management through traditional IaC tools like [Ansible](/docs/netdata-agent/installation/ansible), allowing version-controlled, auditable deployments.
...
1. **[Dynamic Configuration (DynCfg)](/docs/netdata-agent/configuration/dynamic-configuration-manager)** - Runtime configurations via Netdata Cloud UI or API
2. **[User Configuration](/docs/netdata-agent/configuration)** - Files in `/etc/netdata` or `/opt/netdata/etc/netdata`
...
### [Process and Application Monitoring](/docs/agent/src/collectors/apps.plugin/integrations/applications) (apps.plugin)
...
### [Applications](/docs/collecting-metrics/collectors-configuration) (go.d.plugin)
...
- **[Docker container discovery](/docs/collecting-metrics/service-discovery/discoverer/docker) (dockersd)** - Discovers applications running in Docker containers
- **[Kubernetes service discovery](/docs/collecting-metrics/service-discovery/discoverer/kubernetes) (k8ssd)** - Discovers services running in Kubernetes pods
- **[SNMP device discovery](/docs/collecting-metrics/service-discovery/discoverer/snmp) (snmpsd)** - Discovers and profiles SNMP-enabled network devices
...
Supported applications include databases (MySQL, PostgreSQL, Redis, MongoDB), web servers (NGINX, Apache, HAProxy), message queues (RabbitMQ, Kafka), SNMP, and [many more](/docs/collecting-metrics/collectors-configuration).
...
**[Windows-Specific Auto-Discovery](/docs/netdata-agent/installation/windows)**:
...
**Cluster Monitoring**:
- **[Cluster state](/docs/netdata-agent/installation/kubernetes)** (k8s_state): Monitors nodes, pods, containers, deployments, services
- **[Kubelet metrics](/docs/netdata-agent/installation/kubernetes) (k8s_kubelet)**: Container and pod resource usage, volume statistics
- **[Control plane](/docs/netdata-agent/installation/kubernetes)** (kube-proxy, kube-scheduler, kube-controller-manager): When accessible

**[Service Discovery](/docs/agent/src/go/plugin/go.d/discovery/sdext/discoverer/k8ssd)** (k8ssd):
...
Most Kubernetes deployments require no configuration beyond the initial [Helm chart installation](/docs/netdata-agent/installation/kubernetes-helm-chart-reference).
...
- [Parent-Child Configuration Reference](/docs/netdata-parents/parent-child-configuration-reference)
- [Parent Configuration Best Practices](/docs/netdata-parents/parent-configuration-best-practices)
- [Streaming and Routing Reference](/docs/netdata-parents/streaming-routing-reference)
- [Dynamic Configuration Manager](/docs/netdata-agent/configuration/dynamic-configuration-manager)
