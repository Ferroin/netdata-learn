...
Telemetry is **on by default** and carries only anonymized metadata — never raw metrics. See [Anonymous telemetry events](/docs/agent/netdata-agent/configuration/anonymous-telemetry-events) for exactly what is collected and the opt-out methods.

### Agent-Cloud Link (ACLK)

The [ACLK](/docs/agent/src/aclk) is the channel the Agent uses to communicate with Netdata Cloud. It:
...
IP addresses can change without notice and vary based on your geographic location due to CDN-edge servers, so **always prefer domain-based allowlisting**. For the complete firewall allowlist with ports, see [Configure Netdata for cybersecurity platforms](/docs/agent/netdata-agent/configure-netdata-for-cybersecurity-platforms#required-endpoints-and-ports). If your Agent requires a proxy to reach the internet, see [proxy configuration](/docs/agent/src/claim#proxy-configuration).
...
Disable automatic updates with the `--no-updates` flag at install time, or by disabling the updater on an existing install. For a fully disconnected deployment, see [Install Netdata on Offline Systems](/docs/agent/packaging/installer/methods/offline).
