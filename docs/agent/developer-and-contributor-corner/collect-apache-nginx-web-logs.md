...
The [web_log](/docs/agent/src/go/plugin/go.d/collector/weblog) collector is currently compatible
...
Restart Netdata with `sudo systemctl restart netdata`, or the [appropriate method](/docs/agent/netdata-agent/start-stop-restart) for your system. Netdata should pick up your web server's access log and begin showing real-time charts!
...
We do have [extensive documentation](/docs/agent/src/go/plugin/go.d/collector/weblog) on how to build custom parsing for Nginx and Apache logs.
...
[health monitoring documentation](/docs/agent/src/health).
