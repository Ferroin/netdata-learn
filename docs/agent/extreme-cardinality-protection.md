...
Metric metadata, including chart definitions, dimensions, and chart labels, is stored in `netdata-meta.db` in the Agent's [cache directory](/docs/agent/netdata-agent/backup-and-restore-an-agent). The directory varies by operating system and installation method.
...
These outcomes are normal and do not mean the cleanup failed. See the [SQLite configuration reference](/docs/agent/src/daemon/config) for the `auto vacuum` setting.
...
Use the [Netdata Agent configuration guide](/docs/agent/netdata-agent/configuration#edit-configuration-files) to locate and edit `netdata.conf`, then [restart the Agent](/docs/agent/netdata-agent/start-stop-restart) for the changes to take effect.
...
You can also open the **Logs** tab and filter `MESSAGE_ID` by **Netdata extreme cardinality**. For platform-specific log access and permission requirements, see [Netdata Logging](/docs/agent/src/libnetdata/log#platform-specific-log-access).
