...
The `go_expvar` collector is disabled by default. To enable it, use `edit-config` from the Netdata [config directory](/docs/agent/netdata-agent/configuration), which is typically at `/etc/netdata`, to edit the `python.d.conf` file.
...
Change the value of the `go_expvar` setting to `yes`. Save the file and restart the Netdata Agent with `sudo systemctl restart netdata`, or the [appropriate method](/docs/agent/packaging/installer#maintaining-a-netdata-agent-installation) for your system.
...
-   [External plugins - charts](/docs/agent/src/plugins.d#chart)
-   [Chart variables](/docs/agent/src/collectors/python.d.plugin#global-variables-order-and-chart)
...
[External plugins - dimensions](/docs/agent/src/plugins.d#dimension)
