...
Find available collectors in the [Collecting Metrics](/docs/agent/src/collectors) guide and on our [Integrations page](https://www.netdata.cloud/integrations).
...
> **Info**
>
> Enable and configure Go collectors directly through the UI using the [Dynamic Configuration Manager](/docs/agent/netdata-agent/configuration/dynamic-configuration).
...
1. Open `netdata.conf` using [`edit-config`](/docs/agent/netdata-agent/configuration#edit-configuration-files).
...
2. Uncomment the collector's line and set it to `no`:
   ...
   modules:
       xyz_collector: no
   ```
3. [Restart](/docs/agent/netdata-agent/start-stop-restart) the Agent after making changes.
