...
You can also instruct `systemd-cat-native` to log to a remote system, sending the logs to a `systemd-journal-remote` instance running on another server. Check [the manual of systemd-cat-native](/docs/agent/src/libnetdata/log/systemd-cat-native).
...
Storing journal files on a compressed filesystem reduces the footprint. In our tests, a compressed filesystem can save up to 75% of the space required by journal files. How much disk journals are allowed to use, and for how long they are kept, is controlled with journald's disk-usage settings — see [Log Storage and Retention](/docs/agent/logs/log-storage-and-retention).
...
Read [the manual of systemd-cat-native](/docs/agent/src/libnetdata/log/systemd-cat-native).
