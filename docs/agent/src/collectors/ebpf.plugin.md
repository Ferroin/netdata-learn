...
For hands-on configuration and troubleshooting tips see our [tutorial on troubleshooting apps with eBPF metrics](/docs/agent/developer-and-contributor-corner/monitor-debug-applications-ebpf).
...
1. Navigate to the [Netdata config directory](/docs/agent/netdata-agent/configuration#the-netdata-config-directory).

    ```bash
    cd /etc/netdata
    ```

2. Use the [`edit-config`](/docs/agent/netdata-agent/configuration#edit-a-configuration-file-using-edit-config) script to edit `netdata.conf`.
...
2. Use the [`edit-config`](/docs/agent/netdata-agent/configuration#edit-a-configuration-file-using-edit-config) script to edit [`ebpf.d.conf`](https://github.com/netdata/netdata/blob/master/src/collectors/ebpf.plugin/ebpf.d.conf).
...
[`apps.plugin`](/docs/agent/src/collectors/apps.plugin). This integration helps you understand how specific applications
...
[`cgroups.plugin`](/docs/agent/src/collectors/cgroups.plugin). This integration helps you understand how a specific cgroup
...
- `cachestat`: Netdata's eBPF data collector creates charts about the memory page cache. When the integration with
    [`apps.plugin`](/docs/agent/src/collectors/apps.plugin) is enabled, this collector creates charts for the whole host _and_
...
2. Use the [`edit-config`](/docs/agent/netdata-agent/configuration#edit-a-configuration-file-using-edit-config) script to edit a thread configuration file. The following configuration files are available:
...
[simple patterns](/docs/agent/src/libnetdata/simple_pattern). The `ports`, and `ips` settings accept negation (`!`) to deny
...
Next, [restart Netdata](/docs/agent/netdata-agent/start-stop-restart) to see if system memory usage (see the `system.ram` chart) has dropped significantly.
...
The eBPF plugin has a [function](/docs/agent/top-monitoring-netdata-functions) named
