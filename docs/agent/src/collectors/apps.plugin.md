...
In addition, if the [eBPF collector](/docs/agent/src/collectors/ebpf.plugin) is running, your dashboard will also show an
additional [list of charts](/docs/agent/src/collectors/ebpf.plugin#integration-with-appsplugin) using low-level Linux
metrics.
...
file using our [`edit-config`](/docs/agent/netdata-agent/configuration#edit-a-configuration-file-using-edit-config) script.
...
The options can be set in the `netdata.conf` using the [`edit-config` script](/docs/agent/netdata-agent/configuration).
...
[`ebpf.d.conf`](/docs/agent/src/collectors/ebpf.plugin#configure-the-ebpf-collector) file to ensure the eBPF program is enabled.

Also see our [guide on troubleshooting apps with eBPF metrics](/docs/agent/developer-and-contributor-corner/monitor-debug-applications-ebpf) for ideas on how to interpret these charts in a few scenarios.
