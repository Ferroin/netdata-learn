...
You can configure the Agent's Database through the database settings. For a deeper understanding of the Database components, see the [Database overview](/docs/agent/src/database).

## Modes

Use [`edit-config`](/docs/agent/netdata-agent/configuration#edit-configuration-files) to open `netdata.conf` and set your preferred mode:
...
Retention size is enforced **per-tier**, not per Child, so all streaming Children share the Parent's tier quota. For Parent sizing guidance, see [Parent Retention Sizing](/docs/agent/netdata-agent/sizing-netdata-agents/disk-requirements-and-retention#parent-retention-sizing).
...
Retention size limits are soft targets, not hard caps enforced at write time. Actual disk usage can temporarily exceed the configured limit. For the detailed enforcement behavior, see [Retention Size Enforcement](/docs/agent/src/database#retention-size-enforcement).

:::

You can change these limits using [`edit-config`](/docs/agent/netdata-agent/configuration#edit-configuration-files) to open `netdata.conf`:
...
Because a longer `update every` collects fewer samples per unit of time, the same per-tier disk size holds more time of data, so you may optionally reduce `dbengine tier X retention size`. For the full disk-sizing implications, see [Disk Requirements & Retention](/docs/agent/netdata-agent/sizing-netdata-agents/disk-requirements-and-retention).
...
For a detailed breakdown into individual caches, the dedicated dbengine cache charts require **extended pulse statistics**, which are disabled by default. To enable them, use [`edit-config`](/docs/agent/netdata-agent/configuration#edit-configuration-files) to open `netdata.conf` and set:
...
[Restart the Agent](/docs/agent/netdata-agent/start-stop-restart) for the change to take effect. Once enabled, the following charts appear under the **Netdata** section:
