...
> ⚠️ You're checking the documentation for the legacy Agent dashboard. For the current version please check [Accessing Netdata Dashboards](/docs/dashboards-and-charts).
...
Netdata uses an [internal, static-threaded web server](/docs/agent/src/web/server) to host the HTML, CSS, and JavaScript
files that make up the local Agent dashboard. You don't have to configure anything to access it, although you can adjust
[your settings](/docs/agent/src/web/server#other-netdataconf-web-section-options) in the `netdata.conf` file, or run Netdata
behind an [Nginx proxy](/docs/agent/netdata-agent/configuration/running-the-netdata-agent-behind-a-reverse-proxy/running-behind-nginx), and so on.
...
broken down into [contexts](/docs/agent/src/web#contexts) and
[families](/docs/agent/src/web#families).
...
[Increase the metrics retention policy](/docs/agent/netdata-agent/configuration/optimizing-metrics-database/change-metrics-storage) for your node to see more historical
timeframes.
...
[families](/docs/agent/src/web#families) from that section. Netdata automatically
...
For information on creating custom dashboards from scratch, see the [custom dashboards](/docs/agent/src/web/gui/custom) guide.
