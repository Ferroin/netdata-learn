...
For a quick reference on setting up the Parent-Child relationship, see the [Configuration Examples](#complete-configuration-examples) or refer to our comprehensive [Parents: Your Centralization Points](/guides/deployment-with-centralization-points) documentation for more details.
...
On the Parent node, you can [configure retention settings](/docs/agent/src/database/configuration#retention-settings) to control how long metrics are stored.
