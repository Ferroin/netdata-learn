...
You can use health alerts in conjunction with any of Netdata's [collectors](/docs/agent/src/collectors) (see
the [supported collector list](/docs/agent/src/collectors/collectors)) to monitor the health of your systems, containers, and
...
[infrastructure](/docs/agent/netdata-cloud/organize-your-infrastructure-invite-your-team), you must configure each node with the same health configuration
files.
...
- The `[health]` section in `netdata.conf`. By editing the daemon's behavior, you can disable health monitoring
    ...
    [daemon configuration](/docs/agent/src/daemon/config#health-section-options) for a table of
...
Navigate to your [Netdata config directory](/docs/agent/netdata-agent/configuration) and
...
[health management API](/docs/agent/src/web/api/health). The
...
[health management API](/docs/agent/src/web/api/health).
...
To write a new health entity into a new file, navigate to your [Netdata config directory](/docs/agent/netdata-agent/configuration),
...
- A few lines use space-separated lists to define how the entity behaves. You can use `*` as a wildcard or prefix with
    `!` for a negative match. Order is important, too! See our [simple patterns docs](/docs/agent/src/libnetdata/simple_pattern) for
    more examples.
...
The full [database query API](/docs/agent/src/web/api/queries) is supported. In short:

- `METHOD` is one of  the available [grouping methods](/docs/agent/src/web/api/queries#grouping-methods) such as `average`, `min`, `max` etc.
     This is required.
...
Defines the list of labels present on a host. See our [host labels guide](/docs/agent/netdata-agent/configuration/organize-systems-metrics-and-alerts) for
...
See our [simple patterns docs](/docs/agent/src/libnetdata/simple_pattern) for more examples.
...
<!-- > If you don't know how to find the CHART_NAME, you can read about it [here](/docs/agent/src/web#charts). -->
...
### Example 6 - [Anomaly rate](/docs/agent/src/ml#anomaly-rate) based CPU chart alert

Warning if 5 minute rolling [anomaly rate](/docs/agent/src/ml#anomaly-rate) averaged across all CPU dimensions is above 5%, critical if it goes above 20%:
...
### Example 7 - [Anomaly rate](/docs/agent/src/ml#anomaly-rate) based node level alert

Warning if 5 minute rolling [anomaly rate](/docs/agent/src/ml#anomaly-rate) averaged across all ML enabled dimensions is above 5%, critical if it goes above 20%:
...
The `lookup` line will use the `anomaly_rate` dimension of the `anomaly_detection.anomaly_rate` ML chart to calculate the average [node level anomaly rate](/docs/agent/src/ml#anomaly-rate) over the last 5 minutes.

## Troubleshooting

You can compile Netdata with [debugging](/docs/agent/src/daemon#debugging) and then set in `netdata.conf`:
