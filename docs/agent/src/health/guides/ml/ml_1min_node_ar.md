...
This alert is triggered when the [node anomaly rate](/docs/agent/src/ml) exceeds the threshold defined in the [alert configuration](https://github.com/netdata/netdata/blob/master/src/health/health.d/ml.conf) over the most recent 1 minute window evaluated. 

For example, with the default of `warn: $this > 1`, this means that 1% or more of the metrics collected on the node have across the most recent 1 minute window been flagged as [anomalous](/docs/agent/src/ml) by Netdata.
...
3. **Check the anomalies tab**: Check the [Anomaly Advisor tab](/docs/dashboards-and-charts/anomaly-advisor-tab) to see an ordered list of what metrics were most anomalous in the highlighted window.
...
5. **Use Metric Correlations**: Use [metric correlations](/docs/agent/metric-correlations) to see what metrics may have changed most significantly comparing before to the highlighted timeframe.

### Useful resources

1. [Machine learning (ML) powered anomaly detection](/docs/agent/src/ml)
2. [Anomaly Advisor tab](/docs/dashboards-and-charts/anomaly-advisor-tab)
3. [Metric Correlations](/docs/agent/metric-correlations)
