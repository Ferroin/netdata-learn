...
| Enable only specific alerts         | Use [`enabled alarms` without `*`](/docs/agent/src/health/reference#enable-only-specific-alerts-whitelist) |
...
See [Alert Configuration Ordering](/docs/agent/src/health/alert-configuration-ordering) for the full conceptual explanation.

## Where to Put Your Overrides

Put your overrides in your [Netdata config directory](/docs/agent/netdata-agent/configuration) under `health.d/` — files there survive upgrades. The **stock config directory** holds Netdata's built-in alert definitions and is replaced during updates.
...
- [Health Configuration Reference](/docs/agent/src/health/reference)
- [Alert Configuration Ordering](/docs/agent/src/health/alert-configuration-ordering)
