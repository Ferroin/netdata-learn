...
- A single `\n` renders as a SPACE. Use `\n\n` for a new paragraph. Two trailing spaces or a trailing `\` force a
  line break.
- Supported: `#` headings, `**bold**`, `-` lists, `` `code` ``, fenced code blocks, `[links](/docs/agent/src/plugins.d/url)` (open in a new
...
- [Main DynCfg Documentation](/docs/agent/src/daemon/dyncfg) - Core DynCfg system concepts and APIs
- [Plugins.d Protocol](/docs/agent/src/plugins.d) - Complete documentation of the plugins.d protocol
