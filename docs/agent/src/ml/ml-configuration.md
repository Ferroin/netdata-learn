...
You can use Netdata's [Machine Learning](/docs/agent/src/ml) capabilities to detect anomalies in your infrastructure metrics. This feature is enabled by default if your [Database mode](/docs/agent/src/database) is set to `db = dbengine`.

## Enabling or Disabling Machine Learning

To enable or disable Machine Learning capabilities on your node:

1. [Edit `netdata.conf`](/docs/agent/netdata-agent/configuration#edit-configuration-files).
2. In the `[ml]` section:
    - Set `enabled` to `yes` to enable ML.
    - Set `enabled` to `no` to disable ML.
    - Leave it at the default `auto` to enable ML only when [Database mode](/docs/agent/src/database) is set to `dbengine`.
3. [Restart Netdata](/docs/agent/netdata-agent/start-stop-restart).
...
This example assumes three child nodes [streaming](/docs/agent/observability-centralization-points/metrics-centralization-points) to one parent node. It shows different ways to configure ML:
