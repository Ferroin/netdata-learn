...
Netdata's [Machine Learning](/docs/agent/src/ml) capabilities are enabled by default if the [Database mode](/docs/agent/src/database) is set to `db = dbengine`

To enable or disable Machine Learning capabilities on a node:

1. [Edit `netdata.conf`](/docs/agent/netdata-agent/configuration#edit-a-configuration-file-using-edit-config)
2. In the `[ml]` section, set `enabled` to `yes` to enable ML, `no` to disable it, or leave it at the default `auto` to enable ML only when [Database mode](/docs/agent/src/database) is set to `dbengine`
