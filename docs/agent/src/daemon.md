...
[start, stop, or restart the daemon](/docs/agent/netdata-agent/start-stop-restart).
...
You can send commands during runtime via [netdatacli](/docs/agent/src/cli).
...
The `FATAL` and `ERROR` messages will always appear in the logs, and `INFO`can be filtered using [severity level](/docs/agent/src/daemon/config#logs-section-options) option.
...
The `collector.log` is the `stderr` of all [collectors](/docs/agent/src/collectors/collectors)
 run by `netdata`.
...
then [restart Netdata](/docs/agent/netdata-agent/start-stop-restart):
