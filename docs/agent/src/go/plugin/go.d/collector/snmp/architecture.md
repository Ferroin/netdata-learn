...
those references when moving a section. The [profile guide](/docs/agent/src/go/plugin/go.d/collector/snmp/profile-format) owns user-facing profile syntax and
rules. The [diagnostics CLI README](/docs/agent/src/go/tools/snmp-diagnostics#normal-device-evidence) owns tool usage
and evidence interpretation. Topology production remains in the
[SNMP topology architecture](/docs/agent/src/go/plugin/go.d/collector/snmp_topology/architecture).
...
[diagnostic stage interpretation](/docs/agent/src/go/tools/snmp-diagnostics#acquisition-processing-and-samples)
when correlating them. File cadence, retention, and publication remain documented under
[Diagnostic Files And Publication](/docs/agent/src/go/plugin/go.d/collector/snmp_topology/architecture#diagnostic-files-and-publication).
...
[composition](/docs/agent/src/go/plugin/go.d/collector/snmp_topology/architecture#topology-profile-composition).
...
[Underscore-prefixed tags](/docs/agent/src/go/plugin/go.d/collector/snmp/profile-format#underscore-prefixed-tags) do not distinguish chart IDs. Rows that reach
...
Profile authoring details remain in the [table metric guide](/docs/agent/src/go/plugin/go.d/collector/snmp/profile-format#table-metrics-multiple-rows).

## BGP Collection And Retained Rows

Profile syntax and peer identity requirements are documented in [BGP rows](/docs/agent/src/go/plugin/go.d/collector/snmp/profile-format#bgp-rows).
...
Profile syntax, signal definitions, and date formats are documented in [Licensing rows](/docs/agent/src/go/plugin/go.d/collector/snmp/profile-format#licensing-rows).
