...
- **Job relabeling** is operator policy. It runs after the job's `selector` and before
  [chart profiles](/docs/agent/src/go/plugin/go.d/collector/prometheus/profile-format) are selected, so it can bring an
...
- It is matched against the **full metric name, including** the `_bucket`/`_sum`/`_count` suffixes of histograms and
  ...
  [`match`](/docs/agent/src/go/plugin/go.d/collector/prometheus/profile-format#match) works on family base names instead.)
...
format's [`relabeling` section](/docs/agent/src/go/plugin/go.d/collector/prometheus/profile-format#relabeling) for authoring
...
[`fallback_type` section](/docs/agent/src/go/plugin/go.d/collector/prometheus/profile-format#fallback_type) for the full order.
