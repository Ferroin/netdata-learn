...
provide [metric relabeling](/docs/agent/src/go/plugin/go.d/collector/prometheus/relabel) that normalizes its metrics
...
[set up the Prometheus collector job](/docs/agent/src/go/plugin/go.d/collector/prometheus/integrations/prometheus_endpoint)
first.
...
[Chart Template Format](/docs/agent/src/go/plugin/framework/charttpl); inline comments explain each field.
...
1. **Scrape** -- the job scrapes the endpoint. The `selector` job option filters unwanted series
   ([selector syntax](/docs/agent/src/go/pkg/prometheus/selector)).
2. **Job normalization** -- the job's
   [relabeling](/docs/agent/src/go/plugin/go.d/collector/prometheus/relabel) rewrites names and labels. Histogram and
...
|:----------------|:--------:|:------------------------------------------------------------------------------------------------------------------------------------------------------|
| `match`         |   yes    | [Netdata simple pattern](/docs/agent/src/libnetdata/simple_pattern) tested against post-job metric family names. One hit makes the profile applicable. |
...
- It is tested against the **metric family names** of the scraped metrics -- the series name with the
  ...
  [Chart template rules](#chart-template-rules) and [the relabeling block `match`](/docs/agent/src/go/plugin/go.d/collector/prometheus/relabel#match).)
...
- Syntax is a Netdata [simple pattern](/docs/agent/src/libnetdata/simple_pattern): a space-separated list of globs where
...
rule format as job-level [metric relabeling](/docs/agent/src/go/plugin/go.d/collector/prometheus/relabel), including the
...
- This is chart suppression, not ingestion filtering. Use the job `selector` or a
  [relabeling](/docs/agent/src/go/plugin/go.d/collector/prometheus/relabel) `drop` rule when the
...
[Chart Template Format](/docs/agent/src/go/plugin/framework/charttpl) for every group, chart, instance, label, lifecycle,
...
[Netdata config directory](/docs/agent/netdata-agent/configuration).
...
2. Start from the [Complete example](#complete-example) above or from a stock profile (for example
   [`haproxy.yaml`](/docs/agent/src/go/plugin/go.d/config/go.d/prometheus.profiles/default/haproxy.yaml), installed under
