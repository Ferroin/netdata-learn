...
- Run it in the debug mode `bin/godplugin -d -m <MODULE_NAME>`. This will output the `STDOUT` of the collector, the same
  ...
  our [documentation](/docs/agent/src/plugins.d#external-plugins-api).
...
produces [`charts`](/docs/agent/src/plugins.d#chart), not
raw metrics.
...
- `Charts` returns
  the [charts](/docs/agent/src/plugins.d#chart) (`*module.Charts`).
