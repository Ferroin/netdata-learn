...
Operator configuration is documented in [node identities](/docs/docs/learn/node-identities).
...
- Acquisition updates the configured host authority but MUST NOT announce a host by itself. Ordinary job output
  publishes it under the existing [host ownership contract](/docs/agent/src/go/plugin/hostoutput#metadata-ownership).
