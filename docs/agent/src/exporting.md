...
databases](/docs/agent/exporting-metrics), or jump in to [enabling a connector](/docs/agent/exporting-metrics/enable-an-exporting-connector).
...
restart its process_, not the entire [database of long-term metrics](/docs/agent/netdata-agent/configuration/optimizing-metrics-database/change-metrics-storage).
...
[list of supported databases](/docs/agent/exporting-metrics#supported-databases) for information on which
connector to enable and configure for your database of choice.

- [**AWS Kinesis Data Streams**](/docs/agent/src/exporting/aws_kinesis): Metrics are sent to the service in `JSON`
    format.
- [**Google Cloud Pub/Sub Service**](/docs/agent/src/exporting/pubsub): Metrics are sent to the service in `JSON`
    format.
- [**Graphite**](/docs/agent/src/exporting/graphite): A plaintext interface. Metrics are sent to the database server as
    ...
    Graphite](/docs/agent/src/exporting/graphite).
- [**JSON** document databases](/docs/agent/src/exporting/json)
- [**OpenTSDB**](/docs/agent/src/exporting/opentsdb): Use a plaintext or HTTP interfaces. Metrics are sent to
    OpenTSDB as `prefix.chart.dimension` with tag `host=hostname`.
- [**MongoDB**](/docs/agent/src/exporting/mongodb): Metrics are sent to the database in `JSON` format.
- [**Prometheus**](/docs/agent/src/exporting/prometheus): Use an existing Prometheus installation to scrape metrics
    from node using the Netdata API.
- [**Prometheus remote write**](/docs/agent/src/exporting/prometheus/remote_write). A binary snappy-compressed protocol
    buffer encoding over HTTP. Supports many [storage
    providers](https://prometheus.io/docs/operating/integrations/#remote-endpoints-and-storage).
- [**TimescaleDB**](/docs/agent/src/exporting/timescale): Use a community-built connector that takes JSON streams from a
