...
You can view your current configuration, including default values, at `http://IP:19999/netdata.conf`. Access to this URL is [restricted to local IPs by default](/docs/agent/src/web/server#access-lists).
...
| Section                                                     | Controls                                                 |
|-------------------------------------------------------------|----------------------------------------------------------|
| [`[global]`](#configuration-section-details)                | [Daemon](/docs/agent/src/daemon)                          |
| [`[db]`](#configuration-section-details)                    | [Database](/docs/agent/src/database)                      |
...
| `[ml]`                                                      | [Machine Learning](/docs/agent/src/ml)                    |
| [`[health]`](#configuration-section-details)                | [Health monitoring](/docs/agent/src/health)               |
| `[web]`                                                     | [Web Server](/docs/agent/src/web/server)                  |
| `[registry]`                                                | [Registry](/docs/agent/src/registry)                      |
| `[telemetry]`                                               | Internal monitoring                                      |
| `[statsd]`                                                  | [StatsD plugin](/docs/agent/src/collectors/statsd.plugin) |
...
After you modify `netdata.conf`, you need to [restart Netdata](/docs/agent/netdata-agent/start-stop-restart) for your changes to take effect.
...
3. [Restart Netdata](/docs/agent/netdata-agent/start-stop-restart) to apply the changes.
...
|     process scheduling policy      |     `keep`     | See [Netdata process scheduling policy](/docs/agent/src/daemon#process-scheduling-policy-unix-only)                                                                                                                                                                                                                                                                                                                                                              |
|             OOM score              |      `0`       |                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
|                   retention                   |             `3600`             | Used with `db = ram/alloc`, not the default `db = dbengine`. This number reflects the number of entries the `netdata` daemon will by default keep in memory for each chart dimension. Check [Memory Requirements](/docs/agent/netdata-agent/sizing-netdata-agents/disk-requirements-and-retention) for more information.                                                                                                                                                                                                                                                                                                                                   |
|                 storage tiers                 |              `3`               | The number of storage tiers you want to have in your dbengine. Check the tiering mechanism in the [dbengine's reference](/docs/agent/src/database/engine#tiers). You can have up to 5 tiers of data (including the _Tier 0_). This number ranges between 1 and 5.                                                                                                                                                                                                                                                                                                                                                                                        |
...
|         log         |                         `/var/log/netdata`                         | The directory in which the [log files](/docs/agent/src/daemon#logging) are kept.                                                                                                    |
...
Specific Alerts are configured in per-collector config files under the `health.d` directory. For more info, see [health monitoring](/docs/agent/src/health).
...
|             enabled alarms             |                        *                         | Defines which Alerts to load from both user and stock directories. This is a [simple pattern](/docs/agent/src/libnetdata/simple_pattern) list of Alert or template names. Can be used to disable specific Alerts. For example, `enabled alarms =  !oom_kill *` will load all Alerts except `oom_kill`. Omitting the trailing `*` turns this into a whitelist that loads only the listed Alerts — see [Enable Only Specific Alerts](/docs/agent/src/health/reference#enable-only-specific-alerts-whitelist). |
...
Most internal plugins will provide additional options. Check [Internal Plugins](/docs/agent/src/collectors) for more information.
...
|     setting     |                 default                  | info                                                                                                                                                                                         |
|:---------------:|:----------------------------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|  update every   | the value of `[db].update every` setting | The frequency in seconds the plugin should collect values. For more information check the [performance guide](/docs/agent/netdata-agent/configuration/optimize-the-netdata-agents-performance). |
