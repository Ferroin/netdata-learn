...
`allow from` settings are [Netdata simple patterns](/docs/agent/src/libnetdata/simple_pattern): string matches that use `*`
...
cookies by [editing `netdata.conf`](/docs/agent/netdata-agent/configuration#edit-a-configuration-file-using-edit-config):
