...
You can configure Netdata's **central Registry** to provide unified cross-server dashboards. Together with certain browser features, it allows Netdata to provide these dashboards. The Registry operates with [minimal data transfer](/docs/agent/src/registry#communication-with-the-registry), with all communication occurring directly between your web browser and the Registry.

:::info

Read more about it in the [Registry overview](/docs/agent/src/registry).
...
1. Modify `netdata.conf` using [`edit-config`](/docs/agent/netdata-agent/configuration#edit-configuration-files):
...
2. [Restart the Agent](/docs/agent/netdata-agent/start-stop-restart) for the changes to take effect.
...
You can restrict Registry access to specific IP addresses or hostnames using [simple pattern](/docs/agent/src/libnetdata/simple_pattern) matching:
...
To modify cookie settings, edit `netdata.conf` using [`edit-config`](/docs/agent/netdata-agent/configuration#edit-configuration-files):
