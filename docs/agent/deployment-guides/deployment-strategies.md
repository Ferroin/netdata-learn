...
Check out our [configuration documentation](/docs/agent/netdata-agent/configuration) for all available options.
...
Edit `netdata.conf` on the Child using the [edit-config](/docs/agent/netdata-agent/configuration#locate-your-config-directory) script:
...
Edit `stream.conf` on the Child using the [edit-config](/docs/agent/netdata-agent/configuration#locate-your-config-directory) script:
...
This example helps you configure a Parent with multiple [tiers of metrics storage](/docs/agent/src/database#tiers) to store different time ranges at different resolutions, for 10 Children with about 2k metrics each. Retention in each tier is restricted both in time and storage space, whichever is met first. See [retention settings](/docs/agent/src/database/configuration#retention-settings) for fine-tuning retention.
...
Edit `netdata.conf` on the Parent using the [edit-config](/docs/agent/netdata-agent/configuration#locate-your-config-directory) script:
...
Edit `stream.conf` on the Parent using the [edit-config](/docs/agent/netdata-agent/configuration#locate-your-config-directory) script:
...
2. [Change how long Netdata stores metrics](/docs/agent/src/database/configuration#tiers)
