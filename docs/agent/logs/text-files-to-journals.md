...
|:---------------|:-------------------|
| You use `journalctl`, journal-aware tooling, or a SIEM that ingests journal files | Convert files to journal entries with [log2journal](/docs/agent/src/collectors/log2journal) |
| You already run an OpenTelemetry pipeline, or you want Netdata's indexed log store with per-tenant retention and S3 archiving | Ship files through an [OpenTelemetry Collector](/docs/agent/opentelemetry/logs-collection) |
...
[log2journal](/docs/agent/src/collectors/log2journal) reads a text log file and converts each line into a structured
...
- [systemd-cat-native](/docs/agent/src/libnetdata/log/systemd-cat-native) writes the structured entries to the local journal, a
...
best practices](/docs/agent/src/collectors/log2journal#best-practices).
...
Collector](/docs/agent/opentelemetry/logs-collection). To normalize, enrich, or drop records before export, see
...
Logs](/docs/dashboards-and-charts/logs-tab).

## Reference documentation

- [log2journal](/docs/agent/src/collectors/log2journal) — pattern syntax, field manipulation, stock configurations,
performance and security notes.
- [systemd-cat-native](/docs/agent/src/libnetdata/log/systemd-cat-native) — pushing structured entries to local, namespace, and
remote journals.
- [Collect Logs with OpenTelemetry Collector](/docs/agent/opentelemetry/logs-collection) — `file_log` and `journald` receivers.
