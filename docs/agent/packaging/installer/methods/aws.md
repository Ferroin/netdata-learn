...
You can install Netdata on cloud instances to monitor the apps/services running there, or use multiple instances in a [parent-child streaming](/docs/agent/src/streaming) configuration.

For recommended VM sizing and the resource impact of running Netdata on cloud VMs, see [Impact on Cloud VMs](/docs/agent/impact-on-resources).

## Install Netdata on Amazon EKS (Kubernetes)

Amazon Elastic Kubernetes Service (Amazon EKS) is AWS's managed Kubernetes service. To monitor an EKS cluster, follow the dedicated [Kubernetes installation guide](/docs/agent/packaging/installer/methods/kubernetes), which covers deploying Netdata on Kubernetes using Helm.

## Recommended installation method

The best installation method depends on the instance's operating system, distribution, and version. For Linux instances, we recommend the [`kickstart.sh` automatic installation script](/docs/agent/packaging/installer/methods/kickstart).
