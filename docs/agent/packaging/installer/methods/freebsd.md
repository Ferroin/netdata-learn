...
The simplest method is to use the single line [kickstart script](/docs/agent/packaging/installer/methods/kickstart)
...
page](/docs/agent/netdata-agent/configuration/anonymous-telemetry-events).
...
- `--prepare-offline-install-source`: Instead of installing the agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/agent/packaging/installer/methods/offline) for more info.
