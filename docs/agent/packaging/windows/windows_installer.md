...
See the [Parent-Child Configuration Reference](/docs/agent/src/streaming) for the full setup, and the [Streaming Routing Reference](/docs/agent/docs/streaming-routing) for routing options.
...
To start, stop, or restart the service, use the [PowerShell commands described in Service Control](/docs/agent/netdata-agent/start-stop-restart#windows).
...
For the complete `edit-config` workflow and configuration directory layout, see [Netdata Agent Configuration](/docs/agent/netdata-agent/configuration#edit-configuration-files).
...
The MSI uninstall removes the Netdata binaries and the Windows service, but it does **not** remove your metric database, cache, or edited configuration files. To uninstall the Agent and completely delete the data it leaves behind, see [Uninstall Netdata](/docs/agent/packaging/installer/uninstall).

## Related Windows documentation

- [Service Control](/docs/agent/netdata-agent/start-stop-restart#windows) — Start, stop, restart, and check status of the Netdata Agent
- [Switching Install Types and Release Channels on Windows](/docs/agent/install/windows-release-channels)
- [Uninstall Netdata](/docs/agent/packaging/installer/uninstall) — Remove Netdata from Windows, including the metric database and configuration the MSI uninstall leaves behind
