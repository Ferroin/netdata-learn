...
See [VM Templates](/docs/agent/learn/vm-templates) for how to avoid this when cloning VMs.
...
| Property      | Value                                                                                     |
|---------------|-------------------------------------------------------------------------------------------|
| **Directory** | `vnodes/` in your [Netdata config directory](/docs/agent/netdata-agent/configuration) |
...
In addition to the YAML file method, you can create, edit, test, and remove virtual nodes directly from the Netdata UI using [dynamic configuration (dyncfg)](/docs/agent/netdata-agent/configuration/dynamic-configuration). Both methods produce a working vnode that collectors can attach metrics to, though field requirements differ — see the table below. The Vnodes GUI path is available under the go.d plugin's dynamic configuration view.
...
- Virtual nodes defined in YAML files in the `vnodes/` directory of your [Netdata config directory](/docs/agent/netdata-agent/configuration) have a `stock` or `user` source type — attempting to remove them returns an error: only `dyncfg` vnodes can be removed.

- To remove a file-based vnode, delete or edit its YAML file and [restart the Netdata Agent](/docs/agent/netdata-agent/start-stop-restart).
...
To configure a custom hostname, see [Customizing Your Node Name](/docs/agent/src/daemon/config#customizing-your-node-name).
...
See [Node Ephemerality](/docs/agent/nodes-ephemerality) for detailed configuration options.
...
This is normal for Parent nodes receiving data from Children, and for Agents using Virtual Nodes. See [VM Templates](/docs/agent/learn/vm-templates) for implications when cloning VMs.
...
Yes, but it will appear as a new node in Netdata Cloud and Netdata Parents. Delete the GUID file and status backups, then restart Netdata. See [VM Templates](/docs/agent/learn/vm-templates) for the complete procedure.
...
Yes. Edit `netdata.conf` in your [Netdata config directory](/docs/agent/netdata-agent/configuration) and restart Netdata:
...
Use the [`edit-config` script](/docs/agent/netdata-agent/configuration#edit-configuration-files) to safely edit configuration files, then [restart Netdata](/docs/agent/netdata-agent/start-stop-restart).
...
**To preserve data continuity when renaming a vnode**, change only the `hostname` field in its YAML file in the `vnodes/` directory of your [Netdata config directory](/docs/agent/netdata-agent/configuration) and keep the `guid` unchanged. If a true identity change is needed, accept that historical data belongs to the old identity.
...
The GUID for each virtual node is stored in its YAML configuration file in the `vnodes/` directory of your [Netdata config directory](/docs/agent/netdata-agent/configuration). To look it up:
