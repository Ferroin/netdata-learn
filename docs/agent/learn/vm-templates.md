...
- [Node identity](/docs/agent/learn/node-identities#agent-self-identity)
- [Cloud connection](/docs/agent/learn/node-identities#agent-cloud-link-aclk-identity)
- Alert history
...
- **Read first**: [Node Identities](/docs/agent/learn/node-identities) - understand what you're deleting
...
See [Node Ephemerality](/docs/agent/nodes-ephemerality) for full documentation, cleanup rules, and alerting details.
...
|----------|-------|-------------|
| **[Agent Identity](/docs/agent/learn/node-identities#agent-self-identity)** | [GUID file](/docs/agent/learn/node-identities#agent-self-identity), [status backups](/docs/agent/learn/node-identities#status-file-backups) | Node identity |
| **[ACLK Auth](/docs/agent/learn/node-identities#agent-cloud-link-aclk-identity)** | [`cloud.d/`](/docs/agent/learn/node-identities#agent-cloud-link-aclk-identity) directory | Cloud connection, must re-claim |
| **[Node Metadata](/docs/agent/learn/node-identities#parent-children-identities)** | `netdata-meta.db*`, `context-meta.db*` | Node metadata, metric mappings |
...
Verify the service stopped with `Get-Service Netdata`. See [Service Control](/docs/agent/netdata-agent/start-stop-restart) for details.
...
1. Netdata starts, no [GUID](/docs/agent/learn/node-identities#agent-self-identity) found, generates new unique identity
2. If `claim.conf` exists, auto-claims to Cloud
3. Cloud assigns [Node ID](/docs/agent/learn/node-identities#cloud-node-identity), new node appears in your Space
...
Cause: [GUID recovered from status backup](/docs/agent/learn/node-identities#status-file-backups). Netdata checks multiple backup locations before generating a new GUID.
...
Cause: Either clones share the same [Machine GUID](/docs/agent/learn/node-identities#agent-self-identity) (only one can connect at a time), or `stream.conf` wasn't configured in the template.
...
Cause: [Database files kept](/docs/agent/learn/node-identities#multiple-node-identities-in-database) from the template. The template's node identity persists in the metadata.
...
Cause: Two agents have the same [Machine GUID](/docs/agent/learn/node-identities#agent-self-identity). Cloud kicks the older connection offline when the second connects.
...
Identity persists. Netdata only generates a new [GUID](/docs/agent/learn/node-identities#agent-self-identity) when the file AND all [backups](/docs/agent/learn/node-identities#status-file-backups) are missing.
...
Yes. Each clone gets a unique [Machine GUID](/docs/agent/learn/node-identities#agent-self-identity) and [Claimed ID](/docs/agent/learn/node-identities#agent-cloud-link-aclk-identity). They authenticate with the same token but appear as separate nodes.
