...
For a complete explanation of node states, state transitions, and when nodes move between states, see [Node States and Transitions](/docs/agent/netdata-cloud/node-states-and-transitions).
...
For nodes that are part of streaming configurations, see [Nodes Ephemerality](/docs/agent/nodes-ephemerality) for more advanced configuration options.
...
* [Stop the Netdata Agent](/docs/agent/netdata-agent/start-stop-restart) on the node
* Remove or clear any existing `claim.conf` file
* Clear related environment variables on the node

To remove Netdata from the node entirely instead, see [Uninstall Netdata](/docs/agent/packaging/installer/uninstall).
...
For a complete reference on node states and transitions, see [Node States and Transitions](/docs/agent/netdata-cloud/node-states-and-transitions).

For more advanced configuration options with streaming setups, see [Nodes Ephemerality](/docs/agent/nodes-ephemerality).

To avoid removal issues when cloning VMs, see [VM Templates](/docs/agent/learn/vm-templates) for proper identity cleanup.

To understand how node identity affects removal and cleanup, see [Node Identities](/docs/agent/learn/node-identities).
