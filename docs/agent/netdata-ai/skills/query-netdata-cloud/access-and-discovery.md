...
[Choose The Task](/docs/agent/netdata-ai/skills/query-netdata-cloud/skill#choose-the-task) without loading credentials.
...
Configure the value locally as described in [Prerequisites](/docs/agent/netdata-ai/skills/query-netdata-cloud/skill#prerequisites), never in conversation.
...
to print an authentication header. Source: [agents_query_cloud](/docs/agent/netdata-ai/skills/query-netdata-agents/scripts/_lib.sh).
...
After [wrapper setup](/docs/agent/netdata-ai/skills/query-netdata-cloud/skill#prerequisites), capture discovery output privately instead of displaying identities.
...
[Node response fields](/docs/agent/netdata-ai/skills/query-netdata-cloud/query-nodes#per-node-response-fields) distinguish `nd` (node UUID used in `{nodeId}` paths)
from `mg`
