...
[The helper](/docs/agent/netdata-ai/skills/query-netdata-agents/scripts/_lib.sh) locates its checkout through Git. `agents_load_env` sources trusted `<repo>/.env` and
...
Cloud-token flow in [Safe Execution](/docs/agent/netdata-ai/skills/query-netdata-agents/skill#safe-execution), not manual credential handling.
