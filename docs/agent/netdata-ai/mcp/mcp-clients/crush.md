...
3. **For WebSocket or stdio connections: `nd-mcp` bridge** - The stdio-to-websocket bridge. [Find its absolute path](/docs/agent/netdata-ai/mcp#finding-the-nd-mcp-bridge). Not needed for direct HTTP/SSE connections on v2.7.2+.
...
- `NETDATA_MCP_API_KEY` - Your [Netdata MCP API key](/docs/agent/netdata-ai/mcp#finding-your-api-key)
- `/usr/sbin/nd-mcp` - With your [actual nd-mcp path](/docs/agent/netdata-ai/mcp#finding-the-nd-mcp-bridge) (stdio method only)
...
> **💡 Advanced Usage:** Crush can combine observability data with its terminal-based interface for powerful DevOps workflows. Learn about the opportunities and security considerations in [AI DevOps Copilot](/docs/agent/netdata-ai/mcp/mcp-clients/ai-devops-copilot).
