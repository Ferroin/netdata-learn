...
This is an [MCP Connection](/docs/agent/netdata-ai/mcp/mcp-connections): Netdata Cloud reads *from* PagerDuty. It does not send alerts to PagerDuty — for that, see [PagerDuty notifications](/docs/agent/integrations/cloud-notifications/integrations/pagerduty).
...
Select the PagerDuty connection in a conversation, or when creating a report or scheduled investigation. See [Using MCP servers](/docs/agent/netdata-ai/mcp/mcp-connections#using-mcp-servers).
