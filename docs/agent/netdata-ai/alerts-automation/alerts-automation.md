...
| **[Alerts Configuration Manager](/docs/agent/alerts-and-notifications/creating-alerts-with-netdata-alerts-configuration-manager)** | Visual UI wizard for creating and editing alerts | Manual control over every parameter via a guided interface |
| **[Manual config files](/docs/agent/src/health/reference)** | Edit `health.d/*.conf` files directly | Advanced users, automation pipelines, version-controlled configurations |
...
Both. Alerts Automation provides a fully automated, AI-powered path where you describe what you want in plain English and the AI generates, tests, and helps you deploy the alert configuration. You can also configure alerts manually using the [Alerts Configuration Manager](/docs/agent/alerts-and-notifications/creating-alerts-with-netdata-alerts-configuration-manager) UI or by [editing config files directly](/docs/agent/src/health/reference).
...
1. **ML-based anomaly detection** — Every Netdata Agent runs unsupervised machine learning models that learn normal metric behavior and score anomalies in real time. You can create alerts based on anomaly rates. See [ML Anomaly Detection](/docs/agent/ml-ai/ml-anomaly-detection/ml-anomaly-detection).
...
The [Alerts Configuration Manager](/docs/agent/alerts-and-notifications/creating-alerts-with-netdata-alerts-configuration-manager) is a visual UI wizard where you manually select metrics, set thresholds, and configure parameters field by field. Alerts Automation uses AI to generate the entire configuration from a natural language description and lets you validate it against historical data before deployment. Both produce the same alert definitions.

## See also

- [Alert Configuration Reference](/docs/agent/src/health/reference) — full manual configuration syntax
- [Alerts Configuration Manager](/docs/agent/alerts-and-notifications/creating-alerts-with-netdata-alerts-configuration-manager) — visual UI wizard for alert configuration
- [ML Anomaly Detection](/docs/agent/ml-ai/ml-anomaly-detection/ml-anomaly-detection) — machine learning based anomaly scoring
- [Alert Troubleshooting](/docs/agent/troubleshooting/troubleshoot) — AI-powered alert investigation
- [Netdata AI Overview](/docs/agent/category-overview-pages/machine-learning-and-assisted-troubleshooting) — complete AI capabilities guide
