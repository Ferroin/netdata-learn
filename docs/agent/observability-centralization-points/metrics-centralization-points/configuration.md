...
This section assumes you have read the documentation on [how to edit the Netdata configuration files](/docs/agent/netdata-agent/configuration).
...
1. **Parent node**  
   To generate an SSL key and certificate using `openssl`, take a look at the related section around [Securing Netdata Agents](/docs/agent/src/web/server#examples) in our Documentation.
...
For detailed information on resource requirements and optimization strategies, see [Resource Utilization](/docs/netdata-agent/resource-utilization/).
