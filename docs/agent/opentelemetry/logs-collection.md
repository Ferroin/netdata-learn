...
Use these recipes to send systemd journal entries, application log files, Windows event channels, Kubernetes container logs, and the macOS unified log through an OpenTelemetry Collector to Netdata. For network-device syslog, use the dedicated [OpenTelemetry Collector syslog setup](/docs/agent/npm/syslog/otel-collector). For when to centralize a source at all, see [Centralizing Logs with OpenTelemetry](/docs/agent/logs/centralizing-logs-with-opentelemetry).

Before you begin, complete [Ingest OpenTelemetry Metrics and Logs](/docs/agent/opentelemetry/otlp-ingestion). The examples use OpenTelemetry Collector Contrib `0.157.0` and the local, plaintext loopback endpoint from that guide. Use TLS when the Collector and Netdata Agent are on different hosts.
...
[Securing the OTLP Endpoint](/docs/agent/opentelemetry/securing-the-otlp-endpoint). When you combine blocks, list every
...
If formats vary or transformation must happen after several receivers, use the shared [Transformations](/docs/agent/opentelemetry/transformations) guide instead.
...
See [Log Storage and Retention](/docs/agent/logs/log-storage-and-retention) for how tenant retention and offloading work.
