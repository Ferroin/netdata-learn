...
[`edit-config`](/docs/agent/netdata-agent/configuration#edit-configuration-files)) and applied by restarting the
...
- Issue the server certificate from whatever your infrastructure already trusts — an internal CA or your certificate
  ...
  shown in [Collect Logs with OpenTelemetry Collector](/docs/agent/opentelemetry/logs-collection#shared-exporter).
...
See [Log Storage and Retention](/docs/agent/logs/log-storage-and-retention) for per-tenant retention.
