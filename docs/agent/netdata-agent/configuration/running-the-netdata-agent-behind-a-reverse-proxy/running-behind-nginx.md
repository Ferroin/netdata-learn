...
In case Netdata's web server has been [configured to use TLS](/docs/agent/src/web/server#examples), it is
...
If you use Netdata Cloud, [Bearer Token Protection](/docs/agent/netdata-agent/configuration/secure-your-netdata-agent-with-bearer-token) provides authentication with a single setting - no htpasswd files or nginx auth configuration needed.
...
`allow connections from` accepts [Netdata simple patterns](/docs/agent/src/libnetdata/simple_pattern) to match against the
