...
Tier resolution shifts proportionally when you change `[db].update every`, and a longer `update every` lets the same per-tier disk size hold more time of data. See [Update Every and Tier Granularity](/docs/agent/src/database/configuration#update-every-and-tier-granularity) for the full mechanics and the Parent-Child behavior.
...
These limits are fully configurable. See [Changing how long Netdata stores metrics](/docs/agent/src/database/configuration#tiers).

Under sustained archived-instance churn, such as repeatedly creating short-lived containers, the metric metadata database (`netdata-meta.db`) can grow beyond these defaults. See [Extreme Cardinality Protection](/docs/agent/extreme-cardinality-protection) for the cleanup thresholds, timing, and reasons the database file may not shrink immediately.
...
For details about how dbengine enforces retention size limits, see [Retention Size Enforcement](/docs/agent/src/database#retention-size-enforcement).
...
- Adjust retention (optional): see [Change how long Netdata stores metrics](/docs/agent/src/database/configuration#tiers).
