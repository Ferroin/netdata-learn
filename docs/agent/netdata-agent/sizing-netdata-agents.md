...
|:---------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **CPU**        | [1%-5% of a single core](/docs/agent/netdata-agent/sizing-netdata-agents/cpu-requirements) with default settings; up to [5%-20% in production](/docs/agent/impact-on-resources#typical-netdata-resources-usage-on-production-systems) |
| **RAM**        | [100-200 MB](/docs/agent/netdata-agent/sizing-netdata-agents/ram-requirements) on an empty system; [250-350 MB in typical production](/docs/agent/impact-on-resources#typical-netdata-resources-usage-on-production-systems)          |
| **Disk**       | [~4 GiB by default](/docs/agent/netdata-agent/sizing-netdata-agents/disk-requirements-and-retention#default-disk-footprint) (3 GiB metrics plus metadata), configurable per tier                                                   |
| **Privileges** | Root on Linux, or Administrator on Windows, required for installation                                                                                                                                                           |

For multi-node setups that centralize metrics on a Netdata Parent, resource needs scale with the number of Children and retention — see [Parent Configuration Best Practices](/docs/agent/observability-centralization-points/best-practices).
...
2. **Compact Storage Engine**
   ...
   For per-tier on-disk sample sizes, see [Disk Requirements & Retention](/docs/agent/netdata-agent/sizing-netdata-agents/disk-requirements-and-retention).
