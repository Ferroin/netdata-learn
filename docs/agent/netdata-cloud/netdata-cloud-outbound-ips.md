...
- [Webhook notifications](/docs/agent/integrations/cloud-notifications/integrations/webhook) — alert and reachability payloads
  POSTed to your webhook URL.
- [MCP Connections](/docs/agent/netdata-ai/mcp/mcp-connections) — requests to a Custom MCP Server or a self-hosted MCP
  endpoint.
...
[Required endpoints and ports](/docs/agent/netdata-agent/configure-netdata-for-cybersecurity-platforms#required-endpoints-and-ports).
...
- **Combine with authentication.** An IP allowlist is a network control, not an identity control. Keep the
  [webhook authentication mechanism](/docs/agent/integrations/cloud-notifications/integrations/webhook#authentication-mechanisms)
