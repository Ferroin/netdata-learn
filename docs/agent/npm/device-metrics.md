...
- **Your first device** — [Quick Start](/docs/agent/npm/device-metrics/quick-start).
- **Credentials, SNMPv3, many devices, discovery** — [Configuration](/docs/agent/npm/device-metrics/configuration).
- **Understand or extend model recognition** — [SNMP Profile Format](/docs/agent/src/go/plugin/go.d/collector/snmp/profile-format).
- **Many devices or many sites** — [Sizing and Scaling](/docs/agent/npm/device-metrics/sizing-and-scaling).
- **Trust the data** — [Validation](/docs/agent/npm/device-metrics/validation); avoid the common mistakes in [Anti-patterns](/docs/agent/npm/device-metrics/anti-patterns).
- **Something's off** — [Troubleshooting](/docs/agent/npm/device-metrics/troubleshooting).
