...
You can still pull *any* MMDB build (including a custom one) into the resolver by configuring `enrichment.geoip.asn_database` / `geo_database` directly — the downloader is one of several producers; the plugin doesn't care who wrote the MMDB. See the [Custom MMDB Database](/docs/agent/src/crates/netflow-plugin/integrations/custom_mmdb_database) card. If you prefer MaxMind's own updater, run [`geoipupdate`](/docs/agent/src/crates/netflow-plugin/integrations/maxmind_geoip_-_geolite2) and point `enrichment.geoip.asn_database` / `enrichment.geoip.geo_database` at the MMDB files it produces.
...
When the plugin auto-detects MMDBs this way it forces `optional: true` on the geoip stanza — a missing or transiently-unreadable file does not crash the plugin. If you instead set `asn_database` / `geo_database` explicitly in `netflow.yaml`, you control the `optional` flag yourself; see [Configuration](/docs/agent/npm/network-flows/configuration#enrichment).

## What's next

- Per-provider details (refresh cadence, license, schema, attribution requirements):
  - [DB-IP IP Intelligence](/docs/agent/src/crates/netflow-plugin/integrations/db-ip_ip_intelligence) — the default the downloader fetches.
  - [IPtoASN](/docs/agent/src/crates/netflow-plugin/integrations/iptoasn) — public-domain TSV feed; converted to MMDB by this tool.
  - [CAIDA RouteViews Prefix-to-AS](/docs/agent/src/crates/netflow-plugin/integrations/caida_routeviews_prefix-to-as) — prefix-to-AS TSV feed; ASN-only.
  - [MaxMind GeoIP / GeoLite2](/docs/agent/src/crates/netflow-plugin/integrations/maxmind_geoip_-_geolite2) — authenticated MaxMind downloads or MMDB files managed by `geoipupdate`.
  - [IP2Location LITE IP-Country](/docs/agent/src/crates/netflow-plugin/integrations/ip2location_lite_ip-country) — public country-only CSV ZIP feed.
  - [IPDeny Country Zones](/docs/agent/src/crates/netflow-plugin/integrations/ipdeny_country_zones) — country CIDR archive.
  - [IPIP Country Database](/docs/agent/src/crates/netflow-plugin/integrations/ipip_country_database) — country text ZIP feed.
  - [Custom MMDB Database](/docs/agent/src/crates/netflow-plugin/integrations/custom_mmdb_database) — your own MMDB build.
- The enrichment mechanism that consumes these files: [Enrichment](/docs/agent/npm/network-flows/enrichment) (the MMDB shared mechanism section).
- The plugin knobs that point at the files: [Configuration › `enrichment.geoip`](/docs/agent/npm/network-flows/configuration#enrichment).
