...
See the [Decapsulation integration card](/docs/agent/src/crates/netflow-plugin/integrations/decapsulation) for exporter configuration recipes.
...
See [Retention and querying](/docs/agent/npm/network-flows/retention-querying) and [Field reference](/docs/agent/npm/network-flows/field-reference) for the full picture.
...
- [DB-IP IP Intelligence](/docs/agent/src/crates/netflow-plugin/integrations/db-ip_ip_intelligence) — the default that ships with Netdata
- [MaxMind GeoIP / GeoLite2](/docs/agent/src/crates/netflow-plugin/integrations/maxmind_geoip_-_geolite2) — commercial GeoIP2 or free GeoLite2 with attribution
- [IPtoASN](/docs/agent/src/crates/netflow-plugin/integrations/iptoasn) — public-domain ASN + country, hourly cadence
- [Custom MMDB Database](/docs/agent/src/crates/netflow-plugin/integrations/custom_mmdb_database) — your own MMDB build

**BGP routing**
- [BMP (BGP Monitoring Protocol)](/docs/agent/src/crates/netflow-plugin/integrations/bmp_bgp_monitoring_protocol) — routers push BGP updates over TCP
- [bio-rd RIS](/docs/agent/src/crates/netflow-plugin/integrations/bio-rd_-_ripe_ris) — pull BGP data from a bio-rd-compatible RIS gRPC daemon

**Network identity (cloud IP ranges, IPAM)**
- [AWS IP Ranges](/docs/agent/src/crates/netflow-plugin/integrations/aws_ip_ranges) — public AWS prefix list
- [GCP IP Ranges](/docs/agent/src/crates/netflow-plugin/integrations/gcp_ip_ranges) — public GCP prefix list
- [Azure IP Ranges](/docs/agent/src/crates/netflow-plugin/integrations/azure_ip_ranges) — Azure Service Tags (requires an internal mirror)
- [NetBox](/docs/agent/src/crates/netflow-plugin/integrations/netbox) — open-source IPAM / DCIM
- [Generic JSON-over-HTTP IPAM](/docs/agent/src/crates/netflow-plugin/integrations/generic_json-over-http_ipam) — Infoblox, BlueCat, phpIPAM, custom CMDBs

**Static and rule-based**
- [Static Metadata](/docs/agent/src/crates/netflow-plugin/integrations/static_metadata) — per-exporter, per-interface, per-CIDR labels
- [Classifiers](/docs/agent/src/crates/netflow-plugin/integrations/classifiers) — Akvorado-compatible expression rules

**Decapsulation**
- [Decapsulation](/docs/agent/src/crates/netflow-plugin/integrations/decapsulation) — SRv6 and VXLAN inner-packet extraction

### Related concepts

- [Intel Downloader](/docs/agent/npm/network-flows/intel-downloader) — the bundled tool that fetches and converts MMDB files for the auto-detect path.
- [Configuration](/docs/agent/npm/network-flows/configuration) — the full `enrichment` section reference.
- [Field reference](/docs/agent/npm/network-flows/field-reference) — every flow record field, what writes it, what reads it.
- [Retention and querying](/docs/agent/npm/network-flows/retention-querying) — which fields survive into rollups, how queries pick a tier.
