...
The minimum bucket size is **60 seconds**. Zoom in past one minute and the chart silently widens to 60-second buckets. There's no warning — sub-minute jitter just smooths out. For sub-second analysis, flow data is the wrong tool ([microbursts are invisible](/docs/agent/npm/network-flows/anti-patterns)).

## What forces raw tier

Some queries can't use the rollup tiers. They drop to raw tier and inherit raw-tier retention:

- Filtering or grouping by any [raw-only field](/docs/agent/npm/network-flows/retention-querying#what-survives-the-rollup)
...
- [Sankey and Table](/docs/agent/npm/network-flows/visualization/summary-sankey) — The default view; same aggregation, point-in-time.
- [Retention and Querying](/docs/agent/npm/network-flows/retention-querying) — How tiers map to time ranges.
- [Filters and Facets](/docs/agent/npm/network-flows/visualization/filters-facets) — Narrowing the data.
- [Anti-patterns](/docs/agent/npm/network-flows/anti-patterns) — Common misuses to avoid when reading time-series volume.
