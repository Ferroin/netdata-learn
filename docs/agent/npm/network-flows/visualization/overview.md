...
Filtering uses a structured representation (per-field IN-lists) that's easy to encode as a JSON payload but awkward to URL-encode by hand. The dashboard handles this transparently for sharing — if you script your own queries against the function, use JSON-payload requests, not GET-style args. See [Filters and Facets](/docs/agent/npm/network-flows/visualization/filters-facets) for the details.

## Picking the right view

Each panel suits a different question:

- **[Sankey + Table](/docs/agent/npm/network-flows/visualization/summary-sankey)** — "Who's responsible for this traffic right now". Default landing view.
- **[Time-Series](/docs/agent/npm/network-flows/visualization/time-series)** — "How does this change over time". Same top-N as the Sankey, plotted across the window.
- **[Maps and Globe](/docs/agent/npm/network-flows/visualization/maps-globe)** — "Where is this traffic going". Country / state / city / 3D globe variants.
- **[Filters and Facets](/docs/agent/npm/network-flows/visualization/filters-facets)** — Narrowing the data; the same controls all panels share.
- **[Plugin Health Charts](/docs/agent/npm/network-flows/visualization/dashboard-cards)** — Operational metrics for the plugin itself, not the flow data. Open these first when something looks wrong.

## What's next

- [Sankey and Table](/docs/agent/npm/network-flows/visualization/summary-sankey) — Default landing view.
- [Filters and Facets](/docs/agent/npm/network-flows/visualization/filters-facets) — Narrowing controls.
- [Retention and Tiers](/docs/agent/npm/network-flows/retention-querying) — How the four-tier model determines which queries can satisfy which time ranges.
- [Configuration](/docs/agent/npm/network-flows/configuration) — Tuning the query guardrails (`query_max_groups`).
