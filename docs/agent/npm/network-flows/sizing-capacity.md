...
The raw tier is queried directly for any IP-level investigation, full-text search, city / latitude / longitude maps, and anything that filters on a raw-only field (see [Field Reference](/docs/agent/npm/network-flows/field-reference) for which fields survive into rollups). At 50 000 flow records/s sustained, the raw tier produces ~1.7 TB / day of indexed writes that you may also be reading back in real time.
...
6. **Tune kernel UDP buffers** for burst headroom (see [Troubleshooting](/docs/agent/npm/network-flows/troubleshooting)).
7. **For multi-site deployments**, run one Netdata Agent per router or per site rather than central aggregation.

## What's next

- [Configuration](/docs/agent/npm/network-flows/configuration#per-tier-retention) — Per-tier retention schema.
- [Retention and Querying](/docs/agent/npm/network-flows/retention-querying) — How tiers map to queries and the auto-tier-pick rules.
- [Field Reference](/docs/agent/npm/network-flows/field-reference) — Which fields survive into rollups and which are raw-only.
- [Troubleshooting](/docs/agent/npm/network-flows/troubleshooting) — UDP buffer tuning, query timeout, and disk write pressure.
