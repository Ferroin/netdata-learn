...
For the cross-cutting picture — order of evaluation, the `asn_providers` and `net_providers` chains, the MMDB shared mechanism, the static-vs-dynamic composition rules — see the [Enrichment](/docs/agent/npm/network-flows/enrichment) page. Per-method configuration details (URLs, refresh cadence, license, vendor commands) live on the integration cards under flows.enrichment-methods:

- IP intelligence (MMDB): [DB-IP](/docs/agent/src/crates/netflow-plugin/integrations/db-ip_ip_intelligence), [MaxMind GeoIP / GeoLite2](/docs/agent/src/crates/netflow-plugin/integrations/maxmind_geoip_-_geolite2), [IPtoASN](/docs/agent/src/crates/netflow-plugin/integrations/iptoasn), [Custom MMDB](/docs/agent/src/crates/netflow-plugin/integrations/custom_mmdb_database).
- BGP routing: [BMP](/docs/agent/src/crates/netflow-plugin/integrations/bmp_bgp_monitoring_protocol), [bio-rd RIS](/docs/agent/src/crates/netflow-plugin/integrations/bio-rd_-_ripe_ris).
- Network sources: [AWS IP Ranges](/docs/agent/src/crates/netflow-plugin/integrations/aws_ip_ranges), [Azure IP Ranges](/docs/agent/src/crates/netflow-plugin/integrations/azure_ip_ranges), [GCP IP Ranges](/docs/agent/src/crates/netflow-plugin/integrations/gcp_ip_ranges), [NetBox](/docs/agent/src/crates/netflow-plugin/integrations/netbox), [Generic JSON-over-HTTP IPAM](/docs/agent/src/crates/netflow-plugin/integrations/generic_json-over-http_ipam).
- YAML-defined: [Static Metadata](/docs/agent/src/crates/netflow-plugin/integrations/static_metadata), [Classifiers](/docs/agent/src/crates/netflow-plugin/integrations/classifiers), [Decapsulation](/docs/agent/src/crates/netflow-plugin/integrations/decapsulation).
- Operational: [Enrichment Intel Downloader](/docs/agent/npm/network-flows/intel-downloader) — the bundled refresh tool for MMDB providers.
...
- [Retention and Querying](/docs/agent/npm/network-flows/retention-querying) — How the four tiers work and how the dashboard picks one.
- [Sizing and Capacity Planning](/docs/agent/npm/network-flows/sizing-capacity) — How much disk and CPU you need.
- [Validation and Data Quality](/docs/agent/npm/network-flows/validation) — How to confirm the data is right.
- [Troubleshooting](/docs/agent/npm/network-flows/troubleshooting) — When things break.
