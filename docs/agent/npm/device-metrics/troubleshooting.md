...
- **The poll cycle can't finish inside the interval.** If one hub polls too many objects for its `update_every`, every device suffers. Raise `update_every`, stagger devices, or split them across more hubs — see [Sizing and Scaling](/docs/agent/npm/device-metrics/sizing-and-scaling).
...
**Cause and fix.** The matched profile doesn't declare that OID for your model. Extend the relevant base profile or add a custom one under `/etc/netdata/go.d/snmp.profiles/` that collects the OID, following [SNMP Profile Format](/docs/agent/src/go/plugin/go.d/collector/snmp/profile-format).

## What's next

- [Collect Troubleshooting Data](/docs/agent/npm/device-metrics/collect-snmp-troubleshooting-data) — include built-in SNMP evidence in a
  support bundle for metrics, BGP, licensing, or topology investigations. Share it privately; SNMP evidence is unsanitized.
- [Configuration](/docs/agent/npm/device-metrics/configuration) — the options referenced above.
- [Sizing and Scaling](/docs/agent/npm/device-metrics/sizing-and-scaling) — fixing hub-side overload for good.
- [Anti-patterns](/docs/agent/npm/device-metrics/anti-patterns) — the mistakes that cause these problems in the first place.
