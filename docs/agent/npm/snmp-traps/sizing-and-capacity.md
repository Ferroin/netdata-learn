...
For the byte-unit details of `max_size` and rotation, see [Configuration](/docs/agent/npm/snmp-traps/configuration#direct-journal-retention).

## Shipped limits and defaults

Useful starting points and the fixed caps that bound capacity — not capacity promises. For the full configurable option list, see [Configuration](/docs/agent/npm/snmp-traps/configuration#option-map).
...
For how each control reads out in the receiver counters, see [Metrics](/docs/agent/npm/snmp-traps/metrics#receiver-pipeline).
...
For the full quality checklist, see [Validation and Data Quality](/docs/agent/npm/snmp-traps/validation-and-data-quality).

## What's next

- [Configuration](/docs/agent/npm/snmp-traps/configuration) — buffer, retention, storm-control, and OTLP options.
- [Metrics](/docs/agent/npm/snmp-traps/metrics) — the receiver counters to watch during a storm.
- [Alerts](/docs/agent/npm/snmp-traps/alerts) — the default storm and dedup alerts, plus the kernel UDP buffer-drop alert.
- [Troubleshooting](/docs/agent/npm/snmp-traps/troubleshooting) — missing traps, silent loss, and backend failures.
