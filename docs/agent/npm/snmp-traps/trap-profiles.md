...
Use this page when you need to understand why a trap has a specific name, category, severity, message, tag, or profile-derived chart when profile metrics are enabled. For listener setup and hardening, see [Configuration](/docs/agent/npm/snmp-traps/configuration).
...
For the profile YAML schema, see [SNMP Trap Profile Format](/docs/agent/src/go/plugin/go.d/config/go.d/snmp.trap-profiles/profile-format).
...
Profile validation failures are visible as collector errors and profile-load-failure metrics. After editing profiles, check Logs and receiver metrics before assuming a change is active; see [Metrics](/docs/agent/npm/snmp-traps/metrics) for the receiver diagnostics.
...
For field-level query details, see [Usage and Output](/docs/agent/npm/snmp-traps/usage-and-output) and [Field Reference](/docs/agent/npm/snmp-traps/field-reference).
...
The listener-side `profile_metrics` settings are documented in [Configuration](/docs/agent/npm/snmp-traps/configuration#profile-metrics). Rule and chart syntax live in [SNMP Trap Profile Format](/docs/agent/src/go/plugin/go.d/config/go.d/snmp.trap-profiles/profile-format).
...
Only committed traps update profile metrics. For the exact update rule (dedup-suppressed and failed-write traps do not update metrics, and an OTLP export failure does not roll back an already-updated metric), see [Metrics](/docs/agent/npm/snmp-traps/metrics#profile-defined-metrics).
...
For full metric rule syntax, validation rules, and examples, see [SNMP Trap Profile Format](/docs/agent/src/go/plugin/go.d/config/go.d/snmp.trap-profiles/profile-format).

## Next steps

- To configure listener jobs, overrides, outputs, and `profile_metrics`, see [Configuration](/docs/agent/npm/snmp-traps/configuration).
- To query received traps and read profile-enriched output, see [Usage and Output](/docs/agent/npm/snmp-traps/usage-and-output).
- To understand source identity, relay handling, and device context, see [Enrichment](/docs/agent/npm/snmp-traps/enrichment).
- To understand every emitted trap field, see [Field Reference](/docs/agent/npm/snmp-traps/field-reference).
